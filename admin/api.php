...
function addMesse($mysql,$data) {
  $sql = "INSERT INTO  " . DB_PREFIX . "_messen (titel, slug, `text`, bild, link, themenservice, datum, enddatum, sortierung, kontakt_aktiv, presseteam, pronomen) VALUES ('".$data->titel."', '".$data->slug."', '".$data->text."', '".$data->bild."', '".$data->link."', '".$data->themenservice."', '".$data->datum."', '".$data->enddatum."', '".$data->sortierung."','".$data->kontakt_aktiv."','".$data->presseteam."','".$data->pronomen."')";
  return addObject($mysql, $sql);
}
function updateMesse($mysql,$id, $data) {
  $sql = "UPDATE " . DB_PREFIX . "_messen SET titel='".$data->titel."', slug='".$data->slug."', `text`='".$data->text."', bild='".$data->bild."', link='".$data->link."', themenservice='".$data->themenservice."', datum='".$data->datum."', enddatum='".$data->enddatum."', sortierung='".$data->sortierung."', kontakt_aktiv='".$data->kontakt_aktiv."', presseteam='".$data->presseteam."', pronomen='".$data->pronomen."' WHERE id=".$id;
